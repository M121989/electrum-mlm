...
;Variables
  ...
  !define PRODUCT_NAME "Electrum-MLM"
  !define PRODUCT_WEB_SITE "https://github.com/Mktcoin-official/electrum-mlm"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/electrum-mlm-3.1.4-setup.exe"
...
Section
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-mlm-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-mlm-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrum-mlm-${PRODUCT_VERSION}.exe" 0
  #CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\electrum-mlm-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\electrum-mlm-${PRODUCT_VERSION}.exe" 0


  ;Links mtcoin: URI's to Electrum
  WriteRegStr HKCU "Software\Classes\mktcoin" "" "URL:mktcoin Protocol"
  WriteRegStr HKCU "Software\Classes\mktcoin" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\mktcoin" "DefaultIcon" "$\"$INSTDIR\electrum.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\mktcoin\shell\open\command" "" "$\"$INSTDIR\electrum-mlm-${PRODUCT_VERSION}.exe$\" $\"%1$\""
...
Section "Uninstall"
  ...
  DeleteRegKey HKCU "Software\Classes\mktcoin"
