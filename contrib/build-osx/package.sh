...
${genisoimage} \
    ...
    -dir-mode 0755 \
    -apple \
    -o Electrum_mlm_uncompressed.dmg \
    /tmp/electrum-macos/image || fail "Unable to create uncompressed dmg"

dmg dmg Electrum_mlm_uncompressed.dmg electrum-mlm-$VERSION.dmg || fail "Unable to create compressed dmg"
rm Electrum_mlm_uncompressed.dmg

echo "Done."
md5sum electrum-mlm-$VERSION.dmg
