...
The Kivy GUI is used with Electrum-MLM on Android devices. To generate an APK file, follow these instructions.

## 1. Preliminaries

Make sure the current user can write `/opt` (e.g. `sudo chown username: /opt`).

We assume that you already got Electrum-MLM to run from source on this machine,
...
p4a is used to package Electrum-MLM, Python, SDL and a bootstrap Java app into an APK file. 
We patched p4a to add some functionality we need for Electrum-MLM. Until those changes are
...
In the `gui/kivy` directory of Electrum-MLM, run `make theming`.

## 6. Download Electrum-MLM dependencies
```sh
