...
import electrum_mlm
from electrum_mlm.bitcoin import TYPE_ADDRESS
from electrum_mlm import WalletStorage, Wallet
from electrum_mlm_gui.kivy.i18n import _
from electrum_mlm.paymentrequest import InvoiceStore
from electrum_mlm.util import profiler, InvalidPassword
from electrum_mlm.plugins import run_hook
from electrum_mlm.util import format_satoshis, format_satoshis_plain
from electrum_mlm.paymentrequest import PR_UNPAID, PR_PAID, PR_UNKNOWN, PR_EXPIRED
...
from electrum_mlm.util import base_units
...
class ElectrumWindow(App):
    ...
    def _get_bu(self):
        return self.electrum_config.get('base_unit', 'MLM')
    ...
    def on_qr(self, data):
        from electrum_mlm.bitcoin import base_decode, is_address
        data = data.strip()
        if is_address(data):
            self.set_URI(data)
            return
        if data.startswith('mktcoin:'):
            self.set_URI(data)
            return
        # try to decode transaction
        from electrum_mlm.transaction import Transaction
        from electrum_mlm.util import bh2u
        try:
    ...
    def show_pr_details(self, req, status, is_invoice):
        from electrum_mlm.util import format_time
    ...
    def scan_qr(self, on_complete):
        ...
        SimpleScannerActivity = autoclass("org.electrummlm.qr.SimpleScannerActivity")
    ...
    def on_start(self):
        ...
        # bind intent for mktcoin: URI scheme
        if platform == 'android':
            from android import activity
            from jnius import autoclass
            PythonActivity = autoclass('org.kivy.android.PythonActivity')
    ...
    def load_wallet_by_name(self, path):
        ...
        if wallet:
            ...
            Logger.debug('Electrum-MLM: Wallet not found. Launching install wizard')
    ...
    def init_ui(self):
        ...
        Factory.register('AnimatedPopup',
                         module='electrum_mlm_gui.kivy.uix.dialogs')
        Factory.register('QRCodeWidget',
                         module='electrum_mlm_gui.kivy.uix.qrcodewidget')
    ...
    def notify(self, message):
        try:
            ...
            icon = (os.path.dirname(os.path.realpath(__file__))
                    + '/../../' + self.icon)
            notification.notify('Electrum-MLM', message,
                            app_icon=icon, app_name='Electrum-MLM')
        except ImportError:
