...
from electrum_mlm.util import profiler, parse_URI, format_time, InvalidPassword, NotEnoughFunds, Fiat
from electrum_mlm import bitcoin
from electrum_mlm.util import timestamp_to_datetime
...
class SendScreen(CScreen):
    ...
    def set_URI(self, text):
        import electrum_mlm
        try:
            uri = electrum_mlm.util.parse_URI(text, self.app.on_pr)
        except:
            self.app.show_info(_("Not a Mktcoin URI"))
            return
    ...
    def do_save(self):
        ...
        from electrum_mlm.paymentrequest import make_unsigned_request, PaymentRequest
    ...
    def do_send(self):
        if self.screen.is_pr:
            ...
            if not address:
                self.app.show_error(_('Recipient not specified.') + ' ' + _('Please scan a Mktcoin address or a payment request'))
                return
            if not bitcoin.is_address(address):
                self.app.show_error(_('Invalid Mktcoin Address') + ':\n' + address)
                return
            try:
...
class ReceiveScreen(CScreen):
    ...
    def get_URI(self):
        from electrum_mlm.util import create_URI
    ...
    def do_share(self):
        uri = self.get_URI()
        self.app.do_share(uri, _("Share Mktcoin Request"))
